...
export const KeywordFinder: React.FC = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await fetch('http://localhost:3000/api/keywords', {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('gp_token') || 'mock-session-token';
      const res = await fetch('http://localhost:3000/api/keywords/find', {
        method: 'POST',
        headers: { 
  ...
  const handleDelete = async (id: string) => {
    try {
      const token = localStorage.getItem('gp_token') || 'mock-session-token';
      await fetch(`http://localhost:3000/api/keywords/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    } catch {}
    ...
    <div className="p-6 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-5 gap-6 text-slate-300 font-sans">
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-xl space-y-4">
          <div>
            <h2 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Keywords Audit</h2>
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Core Service Topic</label>
              <input 
                type="text" 
                required
                value={service}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Fiverr Category</label>
              <select 
                value={category} 
          ...
          <div className="px-5 py-4 bg-slate-950/60 border-b border-slate-800/60 flex items-center justify-between flex-wrap gap-2">
            ...
            {activeRun && (
              <button 
                ...
                className="px-2.5 py-1.5 rounded-lg border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-white transition-all text-[10px] font-semibold flex items-center gap-1.5"
              >
              ...
              <div className="space-y-6 text-xs">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-850 flex flex-col justify-between">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Average Difficulty</span>
                    ...
                    {allKeywords.map((kw, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2">
                          <div className="w-24 truncate text-[10px] text-slate-400 font-mono text-left">{kw.keyword}</div>
                          <div className="flex-1 bg-slate-900 rounded-full h-3 overflow-hidden relative border border-slate-800">
                            <div 
                              className="bg-gradient-to-r from-emerald-500 to-teal-500 h-full rounded-full transition-all duration-500" 
                              style={{ width: `${widthPercent}%` }}
                            />
                            <span className="absolute right-2 top-[-2px] text-[8px] font-bold text-slate-400 font-mono">Vol: {kw.estimatedSearchVolume}</span>
                          </div>
                ...
                <div className="bg-slate-950/40 border border-slate-850 rounded-xl overflow-hidden">
                  <div className="p-3 bg-slate-950 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keywords Metric Matrix</div>
                      ...
                      <tbody className="divide-y divide-slate-850 font-mono text-[11px]">
                        {[
                          ...
                          <tr key={idx} className="hover:bg-slate-900/40 text-slate-300">
                            ...
                            <td className="p-3 text-[10px] capitalize text-slate-400">{m.type.replace('-', ' ')}</td>
