...
import { 
  ...
  Folder, FolderOpen, Tag, Plus, RefreshCw, Eye, Download, Info 
} from 'lucide-react';
...
export const MediaManager: React.FC = () => {
  ...
  const loadMedia = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/api/social/media', {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const gpToken = localStorage.getItem('gp_token') || 'mock-session-token';
      const res = await fetch(`http://localhost:3000/api/social/media/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${gpToken}` }
      });
      const data = await res.json();
