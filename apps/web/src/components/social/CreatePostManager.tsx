...
export const CreatePostManager: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const accountsRes = await fetch('http://localhost:3000/api/social/accounts', {
      ...
      const campaignsRes = await fetch('http://localhost:3000/api/social/campaigns', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleAiAction = async (action: 'generate' | 'rewrite' | 'suggest', toneTarget?: string, changeLen?: string) => {
    ...
    try {
      const token = localStorage.getItem('gp_token') || 'mock-session-token';
      const res = await fetch('http://localhost:3000/api/social/ai/generate', {
        method: 'POST',
        headers: {
