    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          <form onSubmit={handlePublish} className="bg-white border border-slate-200 rounded-2xl p-6  space-y-5">
            {/* Title / Reference */}
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Internal Title / Reference</label>
              <input
                type="text"
                placeholder="e.g. NextJS release promo post"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="w-full bg-slate-50 border border-slate-850 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500"
              />
            </div>

            {/* Target platforms */}
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Publish Channels</label>
              {accounts.length === 0 ? (
                <div className="p-4 rounded-xl border border-slate-850 bg-slate-50/20 text-xs text-slate-500 flex justify-between items-center">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${
                          isSelected
                            ...
                            : 'bg-slate-50 border-slate-850 text-slate-500 hover:border-slate-200 hover:text-slate-350'
                        }`}
                      >
                        <div className={`w-4 h-4 rounded flex items-center justify-center border ${isSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-750 bg-slate-50'}`}>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Post Body content</label>
              ...
              <textarea
                ...
                rows={6}
                className="w-full bg-slate-50 border border-slate-850 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-650 focus:outline-none focus:border-emerald-500 font-sans leading-relaxed"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">CTA External Link</label>
                <input
                  type="url"
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Marketing Campaign</label>
                <select
                  value={selectedCampaign}
                  onChange={(e) => setSelectedCampaign(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-850 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500"
                >
            ...
            {/* Media Upload Mock */}
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Post Media</label>
              <div className="flex flex-wrap items-center gap-3">
                <button
                  type="button"
                  onClick={mockUploadMedia}
                  className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 border border-slate-850 hover:border-slate-750 text-slate-500 hover:text-slate-200 rounded-xl text-xs transition-all"
                >
                  <Paperclip className="w-3.5 h-3.5" />
                  Attach Media Asset
                </button>
                {mediaUrls.map((url, i) => (
                  <div key={i} className="relative group w-12 h-12 rounded-xl overflow-hidden border border-slate-200">
            ...
            <div className="border-t border-slate-200/60 pt-4 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">Specify posting time calendar coordinates.</p>
                </div>
                <button
                  type="button"
              ...
              {isSchedule && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-2xl bg-white border border-slate-850">
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-slate-500 uppercase">Post Date</span>
                    <input
                      type="date"
                      value={scheduleDate}
                      onChange={(e) => setScheduleDate(e.target.value)}
                      className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs text-white focus:outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-slate-500 uppercase">Post Time</span>
                    <input
                      type="time"
                      value={scheduleTime}
                  ...
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-slate-500 uppercase">Timezone</span>
                    <select
                      value={timezone}
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 ">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <select
                  ...
                  className="bg-slate-50 border border-slate-850 rounded-lg p-2 text-white"
                >
            ...
            {/* Prompt presets */}
            <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200/60 text-[10px] font-semibold text-slate-700">
              <button
                type="button"
                onClick={() => handleAiAction('rewrite')}
                className="px-2.5 py-1 bg-slate-50 hover:bg-slate-50 border border-slate-850 rounded-lg flex items-center gap-1 transition-all"
              >
                Rewrite
              </button>
              <button
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden  flex flex-col h-[400px]">
            {/* Live mockup tabs header */}
            <div className="bg-white border-b border-slate-850 px-4 py-2.5 flex items-center justify-between">
                ...
                {['linkedin', 'facebook', 'instagram', 'bluesky'].map(plat => (
                  <button
                    ...
                    className={`p-1.5 rounded-lg border transition-all ${
                      previewTab === plat
                        ? 'bg-slate-50 border-slate-200 text-white'
            ...
            <div className="p-5 flex-1 overflow-y-auto bg-slate-100 flex items-center justify-center">
              {previewTab === 'linkedin' && (
                <div className="w-full bg-white border border-slate-200 rounded-xl p-4 text-xs space-y-3 shadow-sm max-w-sm">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-bold text-slate-800 flex items-center gap-1">Alex Vance <span className="text-[9px] text-slate-400 font-normal">&bull; 1st</span></p>
                      <p className="text-[9px] text-slate-500">Freelance Web Specialist | GigPilot AI</p>
                    </div>
                  </div>
                  <p className="text-slate-700 whitespace-pre-wrap font-sans leading-relaxed break-words">{content || 'What is on your mind? Post draft details will show up here.'}</p>
                  {link && (
                    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
                      <div className="p-2.5 border-t border-slate-100 space-y-0.5">
                        ...
                        <p className="font-semibold text-slate-700 truncate">{link}</p>
                      </div>
                    </div>
                  )}
                  {mediaUrls.length > 0 && (
                    <img src={mediaUrls[0]} className="w-full h-40 object-cover rounded-lg border border-slate-200" />
                  )}
                </div>
              )}
              ...
              {previewTab === 'facebook' && (
                <div className="w-full bg-white border border-slate-200 rounded-xl p-4 text-xs space-y-3 shadow-sm max-w-sm">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center font-bold text-white text-[11px]">AV</div>
                    <div>
                      <p className="font-bold text-slate-800">Alex Vance Page</p>
                      <p className="text-[9px] text-slate-500">Just now &bull; Public</p>
                    </div>
                  </div>
                  <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{content || 'Post content text...'}</p>
                  {link && (
                    <div className="border border-slate-200 bg-slate-50 rounded overflow-hidden">
              ...
              {previewTab === 'instagram' && (
                <div className="w-full bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm max-w-sm">
                  <div className="p-3 flex items-center gap-2 border-b border-slate-200">
                    ...
                    <span className="font-bold text-[11px] text-slate-800">alexvance_ig</span>
                  </div>
                  <div className="bg-slate-50 h-44 flex items-center justify-center relative">
                    {mediaUrls.length > 0 ? (
                      <img src={mediaUrls[0]} className="w-full h-full object-cover" />
                    ) : (
                      <div className="text-[10px] text-slate-400 font-mono text-center p-4">
                        [Instagram requires photos/videos]<br />Please attach a media asset to preview.
                      </div>
                    )}
                  </div>
                  <div className="p-3 text-xs space-y-1">
                    <p className="text-slate-700"><span className="font-bold mr-1.5 text-slate-800">alexvance_ig</span>{content || 'Caption text...'}</p>
                    {link && <p className="text-sky-600 text-[10px] truncate">{link}</p>}
                  </div>
                </div>
              )}
              ...
              {previewTab === 'bluesky' && (
                <div className="w-full bg-white border border-slate-200 rounded-xl p-4 text-xs space-y-3 shadow-sm max-w-sm">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center font-bold text-white text-[11px]">AV</div>
                    <div>
                      <p className="font-bold text-slate-800">Alex Vance <span className="text-[9px] text-slate-400 font-normal">@alexv.bsky.social</span></p>
                    </div>
                  </div>
                  <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{content || 'Post content text...'}</p>
                  {link && (
                    <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                      <div className="p-2 border-t border-slate-200 text-[10px] space-y-0.5">
                        <span className="text-[8px] text-slate-500 uppercase">Bluesky Link facet</span>
                        <p className="font-semibold text-slate-700 truncate">{link}</p>
                      </div>
                    </div>
                  )}
