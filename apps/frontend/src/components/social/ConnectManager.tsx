    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:text-white bg-slate-50 border border-slate-200 hover:border-slate-700 rounded-xl transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {providersList.map((prov) => {
          ...
          return (
            <div 
              key={prov.name} 
              className={`bg-white  border border-slate-200 rounded-2xl overflow-hidden transition-all duration-300 hover:border-slate-700 flex flex-col justify-between`}
            >
              {/* Header block */}
              <div className={`bg-gradient-to-br ${prov.color} p-5 border-b border-slate-200 relative`}>
                <div className="flex items-start justify-between">
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-850 flex items-center justify-center">
                  ...
                  {connectedAccs.length > 0 ? (
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-800 text-slate-500 font-medium border border-slate-700">
                      Disconnected
                    </span>
                  )}
                </div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">{prov.description}</p>
              </div>

              {/* Connected users block */}
              <div className="p-5 flex-1 flex flex-col justify-between bg-slate-50/10">
                {connectedAccs.length > 0 ? (
                  <div className="space-y-4">
                    {connectedAccs.map((acc) => (
                      <div key={acc.id} className="p-3 rounded-xl bg-white border border-slate-850 space-y-3">
                        <div className="flex items-center gap-3">
                          {acc.avatar ? (
                            <img src={acc.avatar} alt={acc.display_name} className="w-9 h-9 rounded-full object-cover border border-slate-700" />
                          ) : (
                            <div className="w-9 h-9 rounded-full bg-slate-800 flex items-center justify-center font-bold text-slate-500 text-xs">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[9px] text-slate-500 font-semibold border-t border-slate-200/60 pt-2.5">
                          <div>
                            <span className="text-slate-500 block uppercase tracking-wider text-[8px] mb-0.5">Connected</span>
                            <span className="text-slate-700 font-mono">{new Date(acc.created_at).toLocaleDateString()}</span>
                          </div>
                          <div>
                            <span className="text-slate-500 block uppercase tracking-wider text-[8px] mb-0.5">Last Sync</span>
                            <span className="text-slate-700 font-mono">{new Date(acc.last_sync).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                          <div className="col-span-2 mt-1">
                            <span className="text-slate-500 block uppercase tracking-wider text-[8px] mb-0.5">Token Expiry</span>
                            <span className="text-slate-700 font-mono text-[9px] truncate flex items-center gap-1">
                        ...
                        <div className="flex gap-2 border-t border-slate-200/60 pt-2.5">
                          <button 
                            ...
                            className="flex-1 py-1.5 bg-slate-50 hover:bg-slate-100 text-[10px] font-bold text-slate-700 hover:text-white rounded-lg border border-slate-200 hover:border-slate-700 transition-all"
                          >
                            Sync
                          </button>
                          <button 
      ...
      {connectingProvider && (
        <div className="fixed inset-0 bg-white backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D121F] border border-slate-200 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleConnect} className="p-6 space-y-4">
              {connectingProvider === 'dev.to' ? (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Dev.to API Key</label>
                  <input
                    type="password"
                    placeholder="Enter DEV API Token (or type 'mock')"
                    value={code}
                    onChange={(e) => setCode(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-850 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-650 focus:outline-none focus:border-emerald-500"
                    required
                  />
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Mastodon Instance Host</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">OAuth Code / Key</label>
                    <input
                      type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Bluesky Handle & App Password</label>
                  <input
                    type="text"
                ...
                <div className="space-y-4">
                  <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">Clicking connect will simulate redirecting to the official {connectingProvider} permission consent dialog.</p>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Verification Auth Code</label>
                    <input
                      type="text"
              ...
              <div className="flex gap-3 border-t border-slate-850 pt-4 mt-2">
                <button
                  type="button"
                  onClick={() => setConnectingProvider(null)}
                  className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 text-xs font-bold text-slate-700 hover:text-white rounded-xl border border-slate-850 transition-all"
                >
                  Cancel
                </button>
                <button
