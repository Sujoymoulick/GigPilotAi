...
import { 
  ...
  Linkedin, Facebook, Instagram, Globe, Plus, AlertCircle
} from 'lucide-react';
...
export const CalendarManager: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load posts
      const postsRes = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/social/posts', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const postsData = await postsRes.json();
      
      // Load scheduled items to bind providers if any
      const schedRes = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/social/accounts', { // load accounts for filtering
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const campRes = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/social/campaigns', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
