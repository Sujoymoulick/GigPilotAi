    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-slate-50 border border-dashed border-slate-200 rounded-3xl p-12 text-center max-w-lg mx-auto">
          ...
          <h3 className="text-sm font-bold text-slate-800">No Campaigns Active</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {campaigns.map((camp) => (
            <div 
              key={camp.id} 
              className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-slate-300 transition-all flex flex-col justify-between"
            >
              {/* Header card border */}
              <div className="p-5 border-b border-slate-200 relative">
                <span className="absolute top-5 right-5 text-[9px] px-2 py-0.5 rounded bg-slate-100 border border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                  {camp.status}
                </span>
                
                <div className="flex items-center gap-3">
                  <div className="w-4.5 h-4.5 rounded-full" style={{ backgroundColor: camp.color }} />
                  <h3 className="text-sm font-bold text-slate-900 leading-none">{camp.name}</h3>
                </div>
                
                <p className="text-xs text-slate-500 mt-3 leading-relaxed">{camp.description || 'No description provided.'}</p>
              </div>
              ...
              <div className="p-5 bg-slate-50/50 space-y-4">
                ...
                <div className="flex gap-2 border-t border-slate-200 pt-4">
                  <button 
                    ...
                    className="flex-1 py-2 bg-slate-50 hover:bg-slate-100 text-xs font-bold text-slate-700 hover:text-slate-900 rounded-xl border border-slate-200 transition-all flex items-center justify-center gap-1"
                  >
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleCreateOrEdit} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase block">Campaign Name</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-xs text-slate-800 focus:outline-none focus:border-emerald-500"
                    required
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase block">Budget (USD)</label>
                  <input
                    type="number"
                    value={budget}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase block">Campaign Goal</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase block">Color Tag</label>
                  <input
                    type="color"
                    value={color}
                    onChange={(e) => setColor(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl h-10 p-1 text-xs text-slate-800 focus:outline-none cursor-pointer"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="flex gap-3 border-t border-slate-200 pt-4 mt-2">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 text-xs font-bold text-slate-500 hover:text-slate-800 rounded-xl border border-slate-200 transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-lg transition-all"
                >
                  Confirm Details
                </button>
              </div>
