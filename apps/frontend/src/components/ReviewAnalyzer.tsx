...
export const ReviewAnalyzer: React.FC = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/reviews', {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('gp_token') || 'mock-session-token';
      const res = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/reviews/analyze', {
        method: 'POST',
        headers: { 
