...
export const HistoryManager: React.FC = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/history', {
  ...
  const handleClearAll = async () => {
    ...
    try {
      const token = localStorage.getItem('gp_token') || 'mock-session-token';
      await fetch((import.meta.env.PUBLIC_API_URL || 'http://localhost:3000') + '/api/history', {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    } catch {}
